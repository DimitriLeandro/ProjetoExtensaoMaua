			...
			<fieldset id="field_equipamento" style="border: solid 1px; padding: 15px;">
				...
				<p>
					...
					if ($equipamento->getIcDelete() == 0) { ?>
						...
						<button type="button" class="botao" onclick="window.location.href = 'solicitar_manutencao.php?cd_equipamento=<?php echo $equipamento->getCdEquipamento(); ?>';">Solicitar Manutenção</button>
						<?php
						} else {
							echo 'Deletado';
						}?>
					...
					<?php
						...
						elseif ($_GET['btn'] == 2) {?>
							<button type="button" class="botao" onclick="window.location.href = 'atualizar_equipamento.php?cd_equipamento=<?php echo $equipamento->getCdEquipamento(); ?>';">Selecionar</button>
							<?php
						}*/
                    ?>
				</p>
